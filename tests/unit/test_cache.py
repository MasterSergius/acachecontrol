...
def test_add_happy_path(monkeypatch):
    ...
    acache = AsyncCache()
    acache.add(key=('test_method', 'test_url', {'data': 'test_data'}),
               value='test_response',
               headers={'Cache-Control': 'max-age=604800',
                        'Content-Type': 'text/html; charset=UTF-8'})
    cache_key = ('test_method',
        'test_url',
...
def test_add_no_cache():
    ...
    acache.add(key=('test_method', 'test_url', {'data': 'test_data'}),
               value='test_response',
               headers={'Cache-Control': 'max-age=604800,no-cache',
                        'Content-Type': 'text/html; charset=UTF-8'})
    assert len(acache.cache) == 0

    acache.add(key=('test_method', 'test_url', {'data': 'test_data'}),
               value='test_response',
               headers={'Cache-Control': 'max-age=604800,no-store',
                        'Content-Type': 'text/html; charset=UTF-8'})
    assert len(acache.cache) == 0
